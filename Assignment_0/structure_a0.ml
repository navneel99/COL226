...
module A0 : BigInt = struct
    ...
    let eq a1 a2 = let s1,l1 = a1 and s2,l2 = a1 in
                let a = sameSign s1 s2 and b = inteq l1 l2 in
                match a,b with
                    | true,true -> true
                    | false,true -> let l1 = remLeadZeros l1 in if (l1 = [0]) then true else false
                    | _,_ -> false;;
    (* Greater Than *)
    ...
    (* let addres i1 i2 c = 
        if (i1+i2+c-20)<0 then
            if (i1+i2+c-10)<0 then
                i1+i2+c
            else
                i1+i2+c-10
        else
    ...
    (* l1 and l2 will be from LSB to MSB and the answer is also LSB to MSB *)
    let rec addlist l1 l2 carry = 
        match l1,l2,carry with
        |  [], [], x -> if x=0 then [] else [x]
        |  [], x1::xs, x|  x1::xs, [], x -> let a = addres x1 0 x and b = addcarry x1 0 x in a::addlist xs [] b
        |  x1::xs1, x2::xs2, x -> let a = addres x1 x2 x and b = addcarry x1 x2 x in a :: addlist xs1 xs2 b;; *)
    ...
    let rec sublist l1 l2 c = match l1,l2,c with
        |  [],[],_ -> []
        |  x::xs,[],c -> if(x-c = 0)then xs else (x-c)::xs
    ...
    let div a1 a2 = let s1,l1 = a1 and s2,l2 = a2 in
                    let chk = inteq l2 [] in if chk = true then raise (Divide_by_zero "Can't divide by zero. Sorry!") else
    ...
    let mk_big i =  let a = if (i<0) then -i else i and b = if (i<0) then Neg else NonNeg in
