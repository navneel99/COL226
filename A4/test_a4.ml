...
let e = exp_parser "let def Foo:Tbool -> (Tbool * Tbool) = \\X:Tbool.(X,Y) in Foo(T) end" rho ;;
let ans = hastype g e (Ttuple [Tbool; Tbool]);;
(* Testcases *)
...
(* 
(* let e1 = type_parser "Tbool -> ( (Tint -> Tbool) * Tbool)" rho;; *)
let e1 = exp_parser "\\X:Tint.(X+2)(2+1)" rho;;

hastype g e1 (Tint);;            (* should return true *)
(* let e2 = exp_parser "\\M.(W(Z(M)))" rho;; *)
let g = [("Y",Tbool)];;
let e2 = exp_parser "proj(1,2) (if T then (3,T) else (4,F) fi)" rho;;
(* let e2 = exp_parser "\\X:Tint.(X,Y)" rho;; *)
hastype g e2 (Tbool);; (* should return true *)


(* let d1 = def_parser "def U:Tint = X" rho;; *)
(* 
